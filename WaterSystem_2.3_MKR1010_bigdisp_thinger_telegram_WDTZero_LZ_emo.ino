...
#include <TelegramBot.h>          // May not work correctly with MKR 1010 but works good with MKR 1000, if used line 136 in TelegramBot.cpp needs to be commented.
...
const PROGMEM char softwareVersion[6] = "2.3";
...
void doorSwitchFunction() { // Read door switch and take actions as needed
  ...
  if (!currentDoorSwitchState) displayDimTimer = millis();
...
void errorTelegramNotification () { // Telegram bot init
  ...
  if (InternetConnectionAlive && TelegramConnectionAlive) {
    ...
    generateStatusStrings();
    //    int numNewMessages = bot.getUpdates(1); // Used with universal telegram bot library
    //    message m = bot.getUpdates(); // Read new messages
    if (params.faultCode != 0) {
      bot.sendMessage(SECRET_BOT_CHATID, "ERROR notification:\n" + String (faultDescription));
      if (Bot_LastChatID != SECRET_BOT_CHATID) bot.sendMessage(Bot_LastChatID, "ERROR notification:\n" + String (faultDescription));
    }
    else {
      bot.sendMessage(SECRET_BOT_CHATID, String (faultDescription));
      if (Bot_LastChatID != SECRET_BOT_CHATID) bot.sendMessage(Bot_LastChatID, String (faultDescription));
    }
  ...
  if (millis() > Bot_lasttime + Bot_mtbs)  {
    ...
    if (InternetConnectionAlive && TelegramConnectionAlive) {
      ...
      if ( m.chat_id != 0 ) {
        ...
        //
        if (m.text == "/feedback OFF" || m.text == "/feedback SILENT" || m.text == "/silent") {
          localFeedback = false;
          bot.sendMessage(m.chat_id, "Local feedback mode changed to OFF.");
        }
        if (localFeedback) {
          displayDimTimer = millis();
          longBeep();
          ...
          shortBeep();
          if ((millis() - displayDimTimer) >= (displayDimPeriod * 1000) && (displayDimPeriod != 0)) lcd.noBacklight(); else lcd.backlight();
          String messageReceived = "MSG:" + m.text.substring(0, 16);
        ...
        if (m.text == "/status") {
          generateStatusStrings();
          bot.sendMessage(m.chat_id, "== Minutes worked: " + String(params.controllerUptime) + " ==\n" + lastSystemStatus + "\n" + lastEnvironmentStatus);
          softwareWatchdog.clear();
          bot.sendMessage(m.chat_id, lastOperationStatus);
          if (params.faultCode) bot.sendMessage(m.chat_id, faultDescription);
          softwareWatchdog.clear();
          bot.sendMessage(m.chat_id, "\n" + (String(lastControlStatus) + "\n" + String(lastNetworkStatus) + "\n" + String(lastDateTimeStatus) + "\n== End of status =="));
        }
        else if (m.text == "/start") {
          Bot_mtbs = 1000;
          bot.sendMessage(m.chat_id, "Chat mode active - awaiting your command:");
        }
        else if (m.text == "/pressure OFF") {
          commands.pressureMode = 0;
          bot.sendMessage(m.chat_id, "Pressure mode changed to 0 (OFF).");
        }
        else if (m.text == "/pressure ON" || m.text == "/pressure AUTO") {
          commands.pressureMode = 1;
          bot.sendMessage(m.chat_id, "Pressure mode changed to 1 (AUTO).");
        }
        else if (m.text == "/filling OFF") {
          commands.fillingMode = 0;
          bot.sendMessage(m.chat_id, "Filling mode changed to 0 (OFF).");
        }
        else if (m.text == "/filling ALL" || m.text == "/filling A+M") {
          commands.fillingMode = 1;
          bot.sendMessage(m.chat_id, "Filling mode changed to 1 (AUTO+MANUAL).");
        }
        else if (m.text == "/filling AUTO" || m.text == "/filling A") {
          commands.fillingMode = 2;
          bot.sendMessage(m.chat_id, "Filling mode changed to 2 (AUTO ONLY).");
        }
        else if (m.text == "/filling MANUAL" || m.text == "/filling M") {
          commands.fillingMode = 3;
          bot.sendMessage(m.chat_id, "Filling mode changed to 3 (MANUAL ONLY).");
        }
        else if (m.text == "/feedback ON" || m.text == "/feedback VERBOSE" || m.text == "/silent OFF") {
          localFeedback = true;
          bot.sendMessage(m.chat_id, "Local feedback mode changed to ON.");
        }
        else if (m.text.substring(0, 10) == "/minvolume") {
          commands.minWaterVolume = (m.text.substring(11, 14)).toInt();
          bot.sendMessage(m.chat_id, "minWaterVolume changed to " + String(commands.minWaterVolume) + " L.");
        }
        else if (m.text.substring(0, 10) == "/maxvolume") {
          commands.maxWaterVolume = (m.text.substring(11, 14)).toInt();
          bot.sendMessage(m.chat_id, "maxWaterVolume changed to " + String(commands.maxWaterVolume) + " L.");
        }
        else if (m.text.substring(0, 11) == "/debuglevel") {
          commands.debugLevel = (m.text.substring(12, 13)).toInt();
          bot.sendMessage(m.chat_id, "debugLevel changed to " + String(commands.debugLevel));
        }
        else if (m.text.substring(0, 6) == "/ACcal") {
          ACcalibration = (m.text.substring(7, 10)).toInt() * 1.0;
          bot.sendMessage(m.chat_id, "ACcalibration changed to " + String(ACcalibration));
          initEmonCurrentSensor();
        }
        else if (m.text.substring(0, 11) == "/ACsampling") {
          ACsampling = (m.text.substring(12, 16)).toInt();
          bot.sendMessage(m.chat_id, "ACsampling changed to " + String(ACsampling));
        }
        else if (m.text == "/pause") {
          commands.forcePause = true;
          bot.sendMessage(m.chat_id, "Force pause mode started.");
        }
        else if (m.text == "/resume") {
          if (commands.forcePause) {
            commands.forcePause = false;
            bot.sendMessage(m.chat_id, "Resuming from pause mode...");
          }
          else if (params.faultCode) {
            bot.sendMessage(m.chat_id, "Resuming from error...");
            resumeFromErrorFunction();
          }
        }
        else if (m.text == "/autoreset ON") {
          commands.autoReset = true;
          bot.sendMessage(m.chat_id, "autoReset mode set to: " + String (commands.autoReset));
        }
        else if (m.text == "/autoreset OFF") {
          commands.autoReset = false;
          bot.sendMessage(m.chat_id, "autoReset mode set to: " + String (commands.autoReset));
        }
        else if (m.text == "/reset") {
          commands.forceReset = true;
          bot.sendMessage(m.chat_id, "Force reset initiated.");
        }
        else if (m.text == "/help") {
          bot.sendMessage(m.chat_id, "== Supported commands: ==\n/start: Start an active chat with the bot.\n/status: Request actual status report.\n/pressure OFF: Switch pressure pump OFF.\n/pressure ON: Set pressure pump to AUTO mode.\n/pressure AUTO: = /pressure ON");
          softwareWatchdog.clear();
          bot.sendMessage(m.chat_id, "/filling OFF: Switch filling pump OFF.\n/filling ALL: Set filling pump to AUTO+MANUAL mode.\n/filling A+M: = /filling ALL\n/filling AUTO: Set filling pump to AUTO ONLY mode.\n/filling A: = /filling AUTO\n/filling MANUAL: Set filling pump to MANUAL ONLY mode.\n/filling M: = /filling MANUAL");
          softwareWatchdog.clear();
          bot.sendMessage(m.chat_id, "/ACcal XXX: Calibrate AC current.\n/ACsampling XXXX: change AC sampling rate.");
          softwareWatchdog.clear();
          bot.sendMessage(m.chat_id, "/minvolume XXX: Set lowest water level to XXX litres.\n/maxvolume XXX: Set highest water level to XXX litres.\n/pause: Force system into pause mode.\n/resume: Resume system from pause mode or from error.");
          softwareWatchdog.clear();
          bot.sendMessage(m.chat_id, "/autoreset ON: Set autoReset mode to ON.\n/autoreset OFF: Set autoReset mode to OFF.\n/reset: Force system to reset.\n== End of list ==");
        }
        else {
          bot.sendMessage(m.chat_id, m.text); // Reply to the same chat with the same text
          //          bot.sendMessage(m.chat_id, m.text, ""); // Reply to the same chat with the same text
          bot.sendMessage(m.chat_id, "Waiting your command");
